...
M_creer_loop:
	...
	addi s1, s1, 16 # Prochain missil Ã  crÃ©er
	addi s0, s0, -1 
	j M_creer_loop
...
#corps
	...
	lw s1, vec_mis # s1 = adresse de vec_mis (début du tableau)
	li s2, 16 # s2 = taille d'une structure missile (16 bytes)
	
	lw t0, vit_mis # t0 = vitesse du missile (combien d'unités on se déplace)
	lw t4, haut_in_units # t4 = limite basse de l'écran -----
	lw t1 long_mis
	sub t4 t4 t1
	li t1, 0 # t1 = ordonnée de la base de l'image (y=0)

M_deplacer_loop:
	beqz s0, end_M_deplacer_loop # Fin si tous les missiles ont été vus
	...
	# bge t3, t1, M_deplacer_move # Si y >= 0, il est encore visible, on le déplace
...
M_deplacer_skip:
	...
	addi s0, s0, -1 # Décrémenter le compteur
	j M_deplacer_loop
